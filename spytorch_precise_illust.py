...
ap.add_argument("-lr", "--lr", type = float, help="learning rate")
...
def run_snn(inputs):
    ...
    for t in range(nb_steps):
        ...
        c   = (mthr > 0)
        rst[c] = torch.ones_like(mem)[c]#*.9

        new_syn = alpha*syn +h1[t,:]
        new_syn = custom_quant.apply(new_syn, quantization.global_ab)
        new_mem = beta*mem +syn -rst # reset 0.9?
...
scale1 = init_layer_weights(spytorch_util.w1, nb_inputs).to(device)
...
##### just for non quantized
spytorch_util.w1.data = spytorch_util.w1.data * .05
spytorch_util.w2.data = spytorch_util.w2.data * .05
...
results = {'bit_string': bit_string ,'loss_hist': loss_hist, 'output_hist': output_hist, 'lr':quantization.global_lr}

with open('results/snn_illustrate_'+bit_string+'_'+str(quantization.global_lr)+'.pkl', 'wb') as f:
